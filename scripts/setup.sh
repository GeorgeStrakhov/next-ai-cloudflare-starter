...
# Create seed migration file with initial data
print_info "Creating seed migration for initial data..."
ADMIN_EMAIL_LOWER=$(echo "$ADMIN_EMAIL" | tr '[:upper:]' '[:lower:]')

# Find the next migration number
LAST_MIGRATION=$(ls drizzle/*.sql 2>/dev/null | sort | tail -1 | grep -o '[0-9]\{4\}' | head -1)
if [ -n "$LAST_MIGRATION" ]; then
    NEXT_NUM=$(printf "%04d" $((10#$LAST_MIGRATION + 1)))
else
    NEXT_NUM="0001"
fi

cat > "drizzle/${NEXT_NUM}_seed_initial_data.sql" << EOF
-- Seed Initial Data
-- Generated by setup.sh on $(date)
-- This migration seeds default configuration and admin access.
-- Uses INSERT OR IGNORE for idempotency (safe to run multiple times).

-- Seed initial admin email
INSERT OR IGNORE INTO admin_emails (email, created_at)
VALUES ('${ADMIN_EMAIL_LOWER}', unixepoch() * 1000);

-- Seed default auth settings (mode: ${AUTH_MODE})
INSERT OR IGNORE INTO project_settings (key, value, updated_at)
VALUES ('auth', '{"mode":"${AUTH_MODE}"}', unixepoch() * 1000);
EOF

print_success "Seed migration created: drizzle/${NEXT_NUM}_seed_initial_data.sql"
print_info "  Admin email: $ADMIN_EMAIL_LOWER"
...
if pnpm db:migrate:local; then
    print_success "Local database ready (including seed data)"
else
...
echo -e "  ${GREEN}✓${NC} Admin interface at ${BLUE}/admin${NC} (admin: ${BLUE}$ADMIN_EMAIL${NC})"
echo -e "  ${GREEN}✓${NC} Access control at ${BLUE}/admin/access${NC} (mode: ${BLUE}$AUTH_MODE${NC})"
