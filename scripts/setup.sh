...
print_header "🔐 Creating CI/CD API Token for GitHub Actions"

echo "Creating a separate token for GitHub Actions with minimal permissions..."
echo "This token will only have access to deploy workers and run migrations."
echo ""

print_info "Attempting to create CI/CD token..."

# Create CI/CD token with minimal permissions
CICD_TOKEN_RESPONSE=$(curl -s -X POST "https://api.cloudflare.com/client/v4/user/tokens" \
  -H "Authorization: Bearer ${CF_API_TOKEN}" \
  -H "Content-Type: application/json" \
  --data '{
    "name": "'"${PROJECT_NAME_LOWER}"'-github-actions",
    "policies": [{
      "effect": "allow",
      "resources": {
...
# Extract token value
CICD_TOKEN=$(echo "$CICD_TOKEN_RESPONSE" | grep -o '"value":"[^"]*"' | cut -d'"' -f4)

if [ -n "$CICD_TOKEN" ]; then
    print_success "CI/CD token created successfully"
    CLOUDFLARE_API_TOKEN_CICD="$CICD_TOKEN"
else
    print_warning "Automatic CI/CD token creation requires 'API Tokens Edit' permission"
    print_info "You can create it manually at: https://dash.cloudflare.com/profile/api-tokens"
    echo ""
    echo "Required permissions for CI/CD token:"
    echo "  • Account > Workers Scripts > Edit"
    echo "  • Account > Workers Routes > Edit"
    echo "  • Account > D1 > Edit"
    echo "  • Account > Workers R2 Storage > Write"
    echo ""
    print_info "For now, we'll use your setup token (less secure)"
    CLOUDFLARE_API_TOKEN_CICD="$CF_API_TOKEN"
fi
...
# =============================================================================
# INSTALL R2 AND REPLICATE DEPENDENCIES
# =============================================================================

print_header "📦 Installing R2 and Replicate Dependencies"

print_info "Installing @aws-sdk/client-s3, replicate, uuid, slugify..."

pnpm add @aws-sdk/client-s3 replicate uuid slugify
...
gh secret set CLOUDFLARE_API_TOKEN --body "${CLOUDFLARE_API_TOKEN_CICD}"
...
# IMPORTANT NOTES:
# - CLOUDFLARE_API_TOKEN is a dedicated CI/CD token with minimal permissions
#   (only Workers Scripts/Routes, D1, and R2 access - NOT API token creation)
...
# - POSTMARK_API_KEY, EMAIL_FROM, and REPLICATE_API_KEY are shared across environments
...
CLOUDFLARE_API_TOKEN=${CLOUDFLARE_API_TOKEN_CICD}
        ...
        if gh auth status &> /dev/null; then
            ...
            if confirm "Create GitHub repository and set up secrets automatically?"; then
                ...
                if gh repo create "${PROJECT_NAME}" --${REPO_VISIBILITY} --source=. --remote=origin --description="${APP_DESCRIPTION}"; then
                    ...
                    # Push all branches
                    print_info "Pushing branches to GitHub..."
                    git push -u origin main -q
                    git push origin stage -q
                    git push origin prod -q
                    print_success "Pushed all branches (main, stage, prod)"

                    # Set GitHub secrets
                    print_info "Setting GitHub secrets..."
                    gh secret set CLOUDFLARE_API_TOKEN --body "${CLOUDFLARE_API_TOKEN_CICD}" 2>/dev/null
...
echo -e "  ${GREEN}✓${NC} CI/CD API token created (minimal permissions for GitHub Actions)"
...
echo -e "   • Add these 11 secrets from ${BLUE}.github-secrets-setup.txt${NC}:"
echo "     - CLOUDFLARE_API_TOKEN (CI/CD token with minimal permissions)"
